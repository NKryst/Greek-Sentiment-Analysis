...
mood_special<-matrix(nrow = length(fileslist), ncol = 6)
for(e in 1:length(fileslist)){
  ...
  mood_special[e,]<-mood_of_the_day_only_sentiments
}
#Adding a row to mood matrix
mood1<-rbind(mood1,mood_special)
write.csv2(mood_special, file = "mood1.csv")